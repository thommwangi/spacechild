...
<style>
body { 
    background: url("../../images/solarsystem/moon.jpg");
	background-size:100%;
...
<?php 															
																...
																$fetchqry2 = "UPDATE `moonquiz` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `moonquiz` where id='$c'"; 
...
<tr><td><h3><br><?php echo @$row['que'];?></h3></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 6){ ?>
 ...
 <?php if($_SESSION['clicks']>5){ 
	$qry3 = "SELECT `ans`, `userans` FROM `moonquiz`;";
