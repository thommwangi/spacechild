...
if(isset($_GET['delete'])){
    ...
    $link->query("DELETE FROM sunquiz WHERE id='$delete_id'");
    $_SESSION['success_flash'] = 'User has been deleted';
    header('Location: sunquiz.php');
}
...
if($_POST){
    $ansQuery=$link->query("SELECT * FROM sunquiz WHERE ans='$ans'");
    ...
    if(!empty($errors)){
        ...
        $link->query("INSERT INTO sunquiz (que, option_1, option_2,option_3,option_4,ans) VALUES ('$que','$opt1','$opt2','$opt3','$opt4','$ans')");
...
<?php
    ...
    $queQuery = $link->query("SELECT * FROM sunquiz ");
    ?> 
<link href="../nav.css" type="text/css" rel="stylesheet">
<ul>
                <li><a  href="../index.php" >Home</a></li>
                <li><a  href="../admin.php">View Users</a></li>
                <li><a href="sunquiz.php" class="active">Sun Quiz</a></li>
    ...
    <h2>Sun Questions</h2>
    <a href="sunquizadd.php" class="btn btn-success pull-right" id="add-product-btn">Add New Question</a>
    <hr>
        ...
        <?php while($que = mysqli_fetch_assoc($queQuery)): ?> 
            <tr>
                ...
                <?php if($que['id'] == $que['id']): ?>
                    <a href="sunquiz.php?delete=<?=$que['id']; ?>" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-remove-sign"></span></a>
                <?php endif; ?>
                </td>
                <td><?= $que['que'];?></td>
                <td><?= $que['option_1'];?></td>
