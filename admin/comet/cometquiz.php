...
if(isset($_GET['delete'])){
    ...
    $link->query("DELETE FROM cometsquiz WHERE id='$delete_id'");
    $_SESSION['success_flash'] = 'Question has been deleted';
    header('Location: cometquiz.php');
}
...
if($_POST){
    $ansQuery=$link->query("SELECT * FROM cometsquiz WHERE ans='$ans'");
    ...
    if(!empty($errors)){
        ...
        $link->query("INSERT INTO cometsquiz (que, option_1, option_2,option_3,option_4,ans) VALUES ('$que','$opt1','$opt2','$opt3','$opt4','$ans')");
...
<?php
    ...
    $queQuery = $link->query("SELECT * FROM cometsquiz ");
    ?> 
...
<ul>
                ...
                <li><a href="../comet/cometquiz.php" class="active">Comets Quiz</a></li>
    ...
    <h2>Comet Questions</h2>
    <a href="cometquizadd.php" class="btn btn-success pull-right" id="add-product-btn">Add New Question</a>
    <hr>
        ...
        <?php while($que = mysqli_fetch_assoc($queQuery)): ?> 
            <tr>
                ...
                <?php if($que['id'] == $que['id']): ?>
                    <a href="cometquiz.php?delete=<?=$que['id']; ?>" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-remove-sign"></span></a>
                <?php endif; ?>
                </td>
                <td><?= $que['que'];?></td>
                <td><?= $que['option_1'];?></td>
