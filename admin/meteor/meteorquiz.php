...
if(isset($_GET['delete'])){
    ...
    $link->query("DELETE FROM meteoritesquiz WHERE id='$delete_id'");
    $_SESSION['success_flash'] = 'Question has been deleted';
    header('Location: meteorquiz.php');
}
...
if($_POST){
    $ansQuery=$link->query("SELECT * FROM meteoritesquiz WHERE ans='$ans'");
    ...
    if(!empty($errors)){
        ...
        $link->query("INSERT INTO meteoritesquiz (que, option_1, option_2,option_3,option_4,ans) VALUES ('$que','$opt1','$opt2','$opt3','$opt4','$ans')");
...
<?php
    ...
    $queQuery = $link->query("SELECT * FROM meteoritesquiz ");
    ?> 
...
<ul>
                ...
                <li><a href="../meteor/meteorquiz.php" class="active">Meteorites Quiz</a></li>
                <li><a href="../moon/moonquiz.php">Moon Quiz</a></li>
                 <li><a href="../planets/planetsquiz.php">Planets Quiz</a></li>
    </ul>

    <h2>Meteor Questions</h2>
    <a href="meteorquizadd.php" class="btn btn-success pull-right" id="add-product-btn">Add New Question</a>
    <hr>
        ...
        <?php while($que = mysqli_fetch_assoc($queQuery)): ?> 
            <tr>
                ...
                <?php if($que['id'] == $que['id']): ?>
                    <a href="meteorquiz.php?delete=<?=$que['id']; ?>" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-remove-sign"></span></a>
                <?php endif; ?>
                </td>
                <td><?= $que['que'];?></td>
                <td><?= $que['option_1'];?></td>
