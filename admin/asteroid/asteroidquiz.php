...
if(isset($_GET['delete'])){
    ...
    $link->query("DELETE FROM asteroidsquiz WHERE id='$delete_id'");
...
if($_POST){
    $ansQuery=$link->query("SELECT * FROM asteroidsquiz WHERE ans='$ans'");
    ...
    if(!empty($errors)){
        ...
        $link->query("INSERT INTO asteroidsquiz (que, option_1, option_2,option_3,option_4,ans) VALUES ('$que','$opt1','$opt2','$opt3','$opt4','$ans')");
...
<?php
    ...
    $queQuery = $link->query("SELECT * FROM asteroidsquiz ");
    ?> 
...
<ul>
				...
				<li><a href="../asteroid/asteroidquiz.php" class="active">Asteroids Quiz</a></li>
    ...
    <h2>Asteroid Questions</h2>
    <a href="asteroidquizadd.php" class="btn btn-success pull-right" id="add-product-btn">Add New Question</a>
    <hr>
        ...
        <?php while($que = mysqli_fetch_assoc($queQuery)): ?> 
            <tr>
                ...
                <?php if($que['id'] == $que['id']): ?>
                    <a href="asteroidquiz.php?delete=<?=$que['id']; ?>" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-remove-sign"></span></a>
                <?php endif; ?>
                </td>
                <td><?= $que['que'];?></td>
                <td><?= $que['option_1'];?></td>
